...
const fleet = [
  {
    ...
    seats: "12–20 seats",
    img: fleetTravellerVan,
    lines: ["Non-AC: from ₹3,500/day or ₹18/km", "AC: from ₹5,000/day or ₹25/km"],
  },
  {
    name: "Premium Traveller AC",
    seats: "13–17 seats",
    img: fleetTravellerInterior,
    lines: ["Pushback recliners + LED + Screen", "AC: from ₹6,200/day or ₹32/km"],
  },
  {
    name: "Medium Coach",
    seats: "21–32 seats",
    img: heroFleet,
    lines: ["Non-AC: from ₹4,500/day or ₹22/km", "AC: from ₹6,200/day or ₹32/km"],
  },
  {
    name: "Large Tourist Coach",
    seats: "33–45 seats",
    img: fleetTouristCoach,
    lines: ["Non-AC: from ₹5,500/day", "AC: from ₹7,800/day", "Semi-Sleeper: from ₹9,800/day"],
  },
  {
    name: "Luxury Coach",
    seats: "46–54 seats",
    img: fleetTouristCoach,
    lines: ["AC: from ₹11,000/day", "Semi-Sleeper: from ₹13,500/day"],
  },
  {
    name: "Volvo / Super Luxury",
    seats: "55+ seats",
    img: heroFleet,
    lines: ["Semi-Sleeper: from ₹17,000/day", "Luxury: from ₹22,000/day"],
  },
];
    ...
    <>
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {fleet.map((f) => (
            <article key={f.name} className="bg-card rounded-xl overflow-hidden card-lift">
              <img
                src={f.img}
                alt={`${f.name} for hire from Varanasi for pilgrimage and group tours`}
                loading="lazy"
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-2">
            {[
              ...
              <span key={u} className="rounded-full bg-card border px-4 py-2 text-sm font-medium">
                {u}
              </span>
            ))}
          </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14">
        ...
        <p className="text-muted-foreground mt-2">
          Sample fares — exact quotation depends on bus category, halts, and dates.
        </p>
          ...
          <table className="w-full text-sm">
            <thead className="bg-secondary/60 text-left">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold">Up to</th>
              </tr>
            </thead>
            <tbody>
              {routes.map((r) => (
          ...
          <ol className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
            {steps.map((s, i) => (
              <li key={s} className="bg-card rounded-xl p-5 text-center card-lift">
                <div className="mx-auto h-10 w-10 rounded-full bg-saffron text-saffron-foreground flex items-center justify-center font-bold">
                  {i + 1}
                </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14">
        ...
        <p className="mt-3 text-foreground/80 leading-relaxed max-w-4xl">
          We operate bus hire services across Uttar Pradesh, Bihar, Uttarakhand, Delhi NCR,
          Rajasthan, Maharashtra, Tamil Nadu, Andhra Pradesh, Jammu & Kashmir, and international
          routes to Nepal including Kathmandu, Pokhara, Lumbini, Janakpur, and Muktinath. From short
          Varanasi airport transfers to multi-state pilgrimage circuits, our fleet handles every
          scale — see our{" "}
          <Link to="/packages" className="text-saffron hover:underline">
            tour packages
          </Link>{" "}
          for ready-made itineraries.
        </p>
        <div className="mt-8 text-center">
          <Link
            to="/contact"
            className="inline-flex rounded-md bg-saffron px-6 py-3 text-sm font-semibold text-saffron-foreground"
          >
            Get a Quote
          </Link>
        </div>
