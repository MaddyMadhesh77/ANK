  ...
  return (
    <>
      <Hero
        ...
        title={
          <>
            Sacred journeys,
            <br />
            <span className="text-gold">crafted with devotion.</span>
          </>
        }
      ...
      >
        <Link
          to="/packages"
          className="rounded-full bg-gold text-gold-foreground px-7 py-3.5 text-sm font-semibold hover:brightness-105 shadow-xl shadow-black/30 transition"
        >
          Explore Packages
        </Link>
        <Link
          to="/plan-trip"
          className="rounded-full border border-white/50 bg-white/5 backdrop-blur px-7 py-3.5 text-sm font-semibold text-white hover:bg-white/15 transition"
        >
          Plan Your Trip
        </Link>
      </Hero>
          ...
          {[
            ...
            <Reveal key={v} delay={i * 80} className="flex flex-col items-center gap-1">
              ...
              <div className="font-bold text-lg">
                {typeof num === "number" ? <AnimatedCounter end={num} suffix={suffix} /> : label}
              </div>
        ...
        <div className="flex items-end justify-between flex-wrap gap-4 mb-10">
          <div>
            ...
            <p className="text-muted-foreground mt-4 max-w-xl">
              Handpicked journeys, led by Varanasi's most experienced operators.
            </p>
          </div>
          <Link to="/packages" className="text-sm font-semibold text-navy hover:text-saffron">
            View All Packages →
          </Link>
        </div>
          ...
          {[
            {
              ...
              loc: "Delhi",
              quote:
                "Our family Chardham yatra was managed end-to-end — clean coach, punctual driver, and not a single hiccup across 12 days.",
            },
            {
              initials: "MP",
              name: "Meena Patel",
              loc: "Ahmedabad",
              quote:
                "We did the Kashi–Ayodhya–Prayag circuit with 36 relatives. The team handled hotels, darshan slots and meals with remarkable care.",
            },
            {
              initials: "AK",
              name: "Anand Kulkarni",
              loc: "Pune",
              quote:
                "Booked the Nepal circuit for our parents. The Tempo Traveller was premium, drivers were respectful and the itinerary was beautifully paced.",
            },
          ].map((t) => (
            <Reveal key={t.name}>
              <article className="bg-card rounded-xl p-6 card-lift h-full">
                <div className="flex items-center gap-3">
                  <div className="h-12 w-12 rounded-full placeholder-gradient flex items-center justify-center text-xs font-bold">
                    {t.initials}
                  </div>
                  <div>
      ...
      <section className="bg-secondary/60">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
          ...
          <p className="text-center text-muted-foreground mt-2">
            Tours and bus hire across India and Nepal.
          </p>
          <div className="mt-6 flex flex-wrap justify-center gap-2">
            {CITIES.map((c) => (
              <Link
                key={c}
                to="/packages"
                className="rounded-full bg-card px-4 py-2 text-sm font-medium border hover:border-saffron hover:text-saffron transition"
              >
                {c}
              </Link>
            ))}
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mt-4">
            A glimpse of our owned fleet. Read more on the{" "}
            <Link to="/about" className="text-saffron hover:underline">
              About
            </Link>{" "}
            page or{" "}
            <Link to="/bus-hire" className="text-saffron hover:underline">
              hire a bus
            </Link>
            .
          </p>
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-3">
          {[
            {
              src: fleetTouristCoach,
              caption: "AARYA Luxury Tourist Coach — All India Permit",
              alt: "Aarya Neelkamal luxury tourist coach with All India Tourist Permit, parked in Varanasi",
            },
            {
              src: fleetTravellerVan,
              caption: "Jannet Tempo Traveller — Group Pilgrimage",
              alt: "Premium Tempo Traveller for small group pilgrimage tours",
            },
            {
              src: fleetTravellerInterior,
              caption: "AC Cabin with Entertainment & Pushback Seats",
              alt: "Traveller interior showing pushback leather seats, LED roof lighting and entertainment screen",
            },
          ].map((b, i) => (
            <Reveal key={i} delay={i * 90}>
              <figure className="card-lift rounded-2xl overflow-hidden bg-card border group">
                ...
                <figcaption className="p-4 text-sm text-center text-navy font-semibold">
                  {b.caption}
                </figcaption>
              </figure>
            </Reveal>
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20 text-white">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-3 font-display text-3xl sm:text-5xl font-bold text-white">
              Let our 30 years of experience plan your next pilgrimage.
            </h2>
            <div className="gold-divider w-24 mt-5" />
            <p className="mt-5 text-white/85 max-w-xl">
              From Kashi to Kathmandu, family yatra to corporate offsite — we craft journeys that
              travel with care.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                to="/plan-trip"
                className="rounded-full bg-gold text-gold-foreground px-7 py-3.5 text-sm font-semibold hover:brightness-105 shadow-xl"
              >
                Plan Your Trip
              </Link>
              <Link
                to="/contact"
                className="rounded-full border border-white/50 bg-white/5 backdrop-blur px-7 py-3.5 text-sm font-semibold text-white hover:bg-white/15"
              >
                Talk to an Expert
              </Link>
            </div>
