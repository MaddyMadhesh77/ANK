...
const ROUTES: Route[] = [
  ...
  },
  {
    ...
    region: "Krishna Bhoomi",
    image: routeVrindavan,
...
function CTAs({ name }: { name: string }) {
  const wa = buildWhatsAppLink(
    `Hi, I'd like to plan a journey to ${name}. Please share route details and bus availability.`,
  );
  return (
    <div className="mt-4 grid grid-cols-3 gap-1.5">
      <Link
        to="/plan-trip"
        className="rounded-md bg-navy text-navy-foreground text-[11px] font-semibold py-2 text-center hover:bg-navy/90 transition"
      >
        Plan Journey
      </Link>
      <Link
        to="/bus-hire"
        className="rounded-md bg-gold text-gold-foreground text-[11px] font-semibold py-2 text-center hover:brightness-105 transition"
      >
        Book Transport
      </Link>
      <a
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-navy">
            Popular Travel Routes &amp; Daily Services
          </h2>
          <div className="gold-divider w-24 mt-3 mx-auto" />
          <p className="text-muted-foreground mt-4">
            Trusted pilgrimage and group transport across India — from the ghats of Kashi to the
            temples of the South, from the Himalayas to the eastern coast.
          </p>
        </div>
        ...
        <div className="mt-10 rounded-2xl bg-navy text-navy-foreground p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-display text-xl font-bold">
                All India Tour &amp; Transport Services
              </h3>
              <p className="text-sm opacity-85 mt-1">
                Daily routes, group bus hire, pilgrimage circuits & customised family travel —
                across every state.
              </p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2.5">
            <Link
              to="/plan-trip"
              className="rounded-full bg-gold text-gold-foreground px-5 py-2.5 text-sm font-semibold hover:brightness-105"
            >
              Talk to Travel Expert
            </Link>
            <a
              href={buildWhatsAppLink("Hi, I'd like to check bus availability and route options.")}
              target="_blank"
              rel="noopener noreferrer"
